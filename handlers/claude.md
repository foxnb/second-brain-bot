...
| reschedule_event | Перенести конкретное событие по названию | "перенеси встречу с Аней на пятницу" |
| change_event_color | Изменить цвет существующего события | "отметь встречу красным" |
...
| set_task_destination | Настройка куда записывать «дела» | "записывай дела в календарь" |
| defer | Пользователь откладывает действие | "потом", "не сейчас" |
