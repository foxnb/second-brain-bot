...
check_items — отметить элементы выполненными: "взяла молоко", "молоко+", "купил хлеб", "убери молоко" (без указания списка — значит купила/сделала), "молоко купила", "сыр ✓"; items=список элементов (используй именительный падеж: "колбасу"→"колбаса", "молоко"→"молоко")
...
РАЗЛИЧИЕ check_items vs set_item_status: если пользователь просто говорит что сделал/взял/убрал — check_items; если явно называет статус ("в работе", "ожидает", "сделано", "готово" как статус) — set_item_status
...
create_note — сохранить заметку: «заметка: Рецепт пасты — ...», «запомни: ...», «сохрани идею ...»; title=название, description=текст заметки, url=ссылка (или null), tags=теги (массив строк без #, или null)
