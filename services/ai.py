...
- add_to_list/create_list: если в сообщении есть URL(ы) без текстовых элементов (или явно "добавь ссылки/ссылку") — помести КАЖДЫЙ URL как отдельную строку в items (не в поле url); поле url используй только когда к URL есть подпись-название (тогда название → items[i], url → url); если пользователь пишет "Добавь туда ссылки URL1 URL2" — items=["URL1","URL2"]; если один URL с текстовым названием "купить молоко https://…" — items=["купить молоко"], url="https://…"
- create_list: тег/теги в этом контексте — это папка, используй folder=тег; НЕ добавляй поле tags для списков
