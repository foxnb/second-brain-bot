...
async def _send_welcome(update_or_query):
    text = (
        "🗓️ Я Revory — твой личный ассистент для продуктивности!\n\n"
        "📅 Календарь:\n"
        "• «встреча завтра в 15:00» — создать событие\n"
...
async def handle_timezone_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if not tz:
        await update.message.reply_text("❌ Не понял формат. Введи смещение как: +3, -5, +5:30, 0")
        return True
    ...
    conn = await load_calendar_connection(user_id)
    if conn:
        await update.message.reply_text(f"✅ Часовой пояс изменён: {display}\n\nГотово! Просто пиши что нужно сделать.")
    else:
        await update.message.reply_text(f"✅ Часовой пояс установлен: {display}\n\nТеперь подключи календарь: /auth")
    return True
...
async def cmd_timezone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id:
        ...
        if current_tz:
            msg = f"Текущий часовой пояс: {iana_to_display(current_tz)}\n\n"
    keyboard = [
        [InlineKeyboardButton("🇷🇺 Москва (UTC+3)", callback_data="tz_set:Europe/Moscow")],
        [InlineKeyboardButton("🌍 Другой", callback_data="tz_ask_custom")],
    ]
    await update.message.reply_text(msg + "Выбери часовой пояс:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def cmd_disconnect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if success:
        await update.message.reply_text("✅ Google Calendar отключён.\n\nАккаунт и история сохранены. Подключить снова: /auth")
    else:
...
async def auth_callback(request: Request):
    ...
    if success:
        if _telegram_app:
            try:
                await _telegram_app.bot.send_message(chat_id=telegram_id, text="✅ Google Calendar успешно подключён! Теперь просто пиши что нужно сделать.")
            except Exception as e:
                logger.error(f"Failed to notify user {telegram_id}: {e}")
        return HTMLResponse("<h2>✅ Готово! Google Calendar подключён.</h2><p>Можешь закрыть эту страницу и вернуться в Telegram.</p>")
    else:
        return HTMLResponse("<h2>❌ Что-то пошло не так.</h2><p>Попробуй ещё раз — отправь /auth в боте.</p>", status_code=500)
...
async def handle_text_wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not tz:
        ...
        await update.message.reply_text("⏰ Сначала установи часовой пояс:", reply_markup=InlineKeyboardMarkup(keyboard))
        return
                ...
                try:
                    ...
                    for r in reminders:
                        try:
                            ...
                            await telegram_app.bot.send_message(chat_id=tid, text=f"🔔 Напоминание: {r['title']}")
